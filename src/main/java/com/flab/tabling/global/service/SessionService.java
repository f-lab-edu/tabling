...
public class SessionService {
	...
	private Object getValue(HttpSession session, SessionConstant key) {
		if (session == null) {
			throw new SessionNotFoundException(ErrorCode.SESSION_NOT_FOUND, "session is not found");
		}
		Object value = session.getAttribute(key.name());
		if (value == null) {
			throw new SessionNotFoundException(ErrorCode.SESSION_NOT_FOUND, "the value of key is not found");
		}
		return value;
	}
